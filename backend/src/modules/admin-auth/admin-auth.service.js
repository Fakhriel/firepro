...
async function login(username, password) {
  ...
  return {
    token,
    admin: { id: admin.id, username: admin.username, name: admin.name, role: admin.role, phone: admin.phone, email: admin.email },
  };
}
...
async function getById(id) {
  ...
  return { id: admin.id, username: admin.username, name: admin.name, role: admin.role, phone: admin.phone, email: admin.email };
}
...
async function create(body) {
  ...
  if (role !== undefined && !MANAGEABLE_ROLES.includes(role)) {
    if (role === 'owner') {
      throw badRequest('Role Owner bersifat tetap dan tidak bisa dibuat lewat panel admin.');
    }
    throw badRequest(`role harus salah satu dari: ${MANAGEABLE_ROLES.join(', ')}.`);
  }
...
async function update(id, body) {
  ...
  const { username, name, role, phone, email } = body;
  const payload = {};
  ...
  if (role !== undefined) {
    if (!MANAGEABLE_ROLES.includes(role)) {
      if (role === 'owner') {
        throw badRequest('Role Owner bersifat tetap dan tidak bisa di-assign lewat panel admin.');
      }
      throw badRequest(`role harus salah satu dari: ${MANAGEABLE_ROLES.join(', ')}.`);
    }
    payload.role = role;
  ...
  if (phone !== undefined) {
    payload.phone = phone ? String(phone).trim() : null;
  }
  if (email !== undefined) {
    if (email && !/^\S+@\S+\.\S+$/.test(String(email).trim())) {
      throw badRequest('Format email tidak valid.');
    }
    payload.email = email ? String(email).trim() : null;
  }
