...
async function create(body) {
  ...
  if (role !== undefined && !['admin', 'superadmin'].includes(role)) {
    throw badRequest('role harus "admin" atau "superadmin".');
  }
...
async function update(id, body) {
  ...
  if (role !== undefined) {
    if (!['admin', 'superadmin'].includes(role)) throw badRequest('role harus "admin" atau "superadmin".');
    payload.role = role;
  }
...
async function changePassword(id, body, requester) {
  ...
  // Kalau ganti password sendiri (bukan superadmin mereset punya orang lain),
...
module.exports = { login, getById, list, create, update, remove, changePassword };